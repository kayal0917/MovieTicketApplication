...
public class MovieImp implements UserDAO {
	...
	public String adminpassword(String username) {
		String adminPassword = "select password from users where user_name=? and delete_user=0";
	...
	public List<ShowTime> fetchShowList(int theaterId) {
		String fetchShowListSql = "SELECT s.showtime_id, m.title AS movieName, s.theater_id, s.show_date, s.show_time "
				+ "FROM show_table s " + "JOIN movie_details m ON s.movie_id = m.movie_id WHERE s.theater_id="+theaterId;
	...
	public List<MovieDetails> getShowDetails(String location) {
		String select = "SELECT " +
                ...
                "    st.show_date, " +
                "    st.show_time, " +
                "    t.delete_user AS theater_delete_user, " +
                "    t.admin_id " +
                "FROM " +
                "    show_table st " +
                "INNER JOIN " +
                ...
                "INNER JOIN " +
                "    movie_details md ON st.movie_id = md.movie_id " +
                "LIMIT 0, 1000;";
		List<MovieDetails> detailsList = jdbcTemplate.query(select, new ShowDetailsMapper());
		return detailsList;
	}
//	  public List<Theater> findAllTheatersWithShowTimes() {
//	        String sql = "SELECT t.theater_id, t.city_id, t.theater_name, t.admin_id, s.showtime_id, s.movie_id, s.show_date, s.show_time "
//	                   + "FROM theater t "
//	                   + "LEFT JOIN show_time s ON t.theater_id = s.theater_id "
//	                   + "ORDER BY t.theater_id, s.showtime_id";
//
//	        List<Theater> theaters = jdbcTemplate.query(sql, new ShowTimesMapper());
//
//	        return theaters;
//	    }		
